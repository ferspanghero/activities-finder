...
- `scripts/` — Python scripts (source fetching, HTML export)
- `tests/` — pytest test suites
- `project_files/` — project documentation (v1/, v2/ subdirectories + backlog)
- `.claude/commands/` — slash command definitions

## Key Files

- `scripts/fetch_sources.py` — async parallel fetcher for all event sources
- `scripts/generate_html.py` — generates self-contained HTML report from JSON
- `tests/test_fetch_sources.py` — unit + integration tests for fetch script
- `tests/test_generate_html.py` — tests for HTML generator
- `.claude/commands/find-activities.md` — `/find-activities` slash command definition
- `pytest.ini` — test configuration

## Commands

- `python3 -m scripts.fetch_sources --date YYYY-MM-DD` — fetch raw source data
- `echo '<json>' | python3 -m scripts.generate_html --output FILE` — generate HTML report
...
Curated sources fetched by `scripts/fetch_sources.py`. No broad web search.
...
### HTML export
Also exports `events-YYYY-MM-DD.html` — self-contained HTML with sortable columns and clickable links. Same table structure. Generated by `scripts/generate_html.py`.
